...
- Authn/z on Live MJPEG and any future WebSocket; a camera credential vault (today the source may sit in `config.yaml` or `data/settings.json`)
...
- If `api_token` is set, the Details camera form, Events list/media, and Incident/Normal buttons need that token; the unauthenticated UI will get 401.
